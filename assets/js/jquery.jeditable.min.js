					...
					return settings.tooltip && $(this).attr("title", settings.tooltip), this.each(function () {
						...
						$(this).data("event.editable", settings.event), $.trim($(this).html()) || $(this).html(settings.placeholder), "destroy" !== target ? ($(this).bind(settings.event, function (e) {
							if (!0 !== $(this).data("disabled.editable") && 9 !== e.which && !self.editing && !1 !== onedit.apply(this, [settings, self, e])) {
								if (settings.before && jQuery.isFunction(settings.before)) settings.before();
								else if (settings.before && !jQuery.isFunction(settings.before)) throw "The 'before' option needs to be provided as a function!";
								...
								var input_content, t, input = element.apply(form, [settings, self]);
								...
								if (settings.loadurl) {
									...
									var loaddata = {};
									console.log('loaddata: ' + loaddata[1]);
								...
								buttons.apply(form, [settings, self]), settings.showfn && $.isFunction(settings.showfn) && form.hide(), $(self).html(""), $(self).append(form), settings.showfn && $.isFunction(settings.showfn) && settings.showfn(form), plugin.apply(form, [settings, self]), form.find(":input:visible:enabled:first").focus(), settings.select && input.select(), $(this).keydown(function (e) {
									...
									if (isSubmitting = !0, t && self.clearTimeout(t), !1 !== onsubmit.apply(form, [settings, self]) && !1 !== submit.apply(form, [settings, self]))
										if ($.isFunction(settings.target)) {
											var responseHandler = function (value) {
													$(self).html(value), self.editing = !1, callback.apply(self, [self.innerHTML, settings]), $.trim($(self).html()) || $(self).html(settings.placeholder)
												},
												userTarget = settings.target.apply(self, [input.val(), settings, responseHandler]);
											!1 !== userTarget && void 0 !== userTarget && responseHandler(userTarget)
										} else {
											var submitdata = {};
											...
											var ajaxoptions = {
												...
												success: function (result, status) {
													var result_valueurl = result.split(',');
													var result_value = result_valueurl[0];
													var result_url = $.trim(result_valueurl[1]);

													if (result_value.length >= 9) {
														alert('Please keep your title to less than 9 characters');
														var result_value = ' ';
														var result_url = ' ';
														self.editing = !1;
														reset.apply(form, [settings, self]);
													} else {
														if (result_url.indexOf("http") >= 0){
														  // console.log("with http: " + result_url);
														} else {
														  // console.log("without http: " + result_url);
															var strhttp = 'http://';
															var result_url = strhttp + result_url;
														  // console.log("now with http: " + result_url);
														}
														result = intercept.apply(self, [result, status]), "html" === ajaxoptions.dataType && $(self).html(result_value), $(self).parent('a').attr("href", result_url), self.editing = !1, callback.apply(self, [result, settings, submitdata]), $.trim($(self).html()) || $(self).html(settings.placeholder)
													}
												},
							...
							self.editing && !1 !== onreset.apply(form, [settings, self]) && ($(self).html(self.revert), self.editing = !1, $.trim($(self).html()) || $(self).html(settings.placeholder), settings.tooltip && $(self).attr("title", settings.tooltip))
							...
							var timeouts = $(self).data("timeouts");
							if (timeouts) {
								for (var i = 0, n = timeouts.length; i < n; ++i) clearTimeout(timeouts[i]);
		...
		types: {
			...
			select: {
				...
				content: function (data, settings, original) {
					...
					var key, option, tuples = [];
					for (key in json) tuples.push([key, json[key]]);
